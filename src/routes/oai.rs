...
use crate::types::{ChatAgentInputs, ChatAgentOutputs, LaminarEndpoints, LaminarOutputs, LaminarRequestArgs, SimpleLLMQueryOutputs};
use crate::AppState;
...
async fn chat(
    ...
    // info!("messages: {:?}", request_args.messages);

    let response = query_laminar_endpoint(
        &app_config,
        &LaminarEndpoints::ChatAgentQuery(ChatAgentInputs {
            messages: request_args.messages,
            user_memories: "".to_string(),
            date: "".to_string()
        }), 
    )
    .await
    .map_err(|e| {
        error!("Error querying laminar: {:?}", e);
        actix_web::error::ErrorInternalServerError(e.to_string())
    })?;
...
async fn query_laminar_endpoint(
    ...
    info!("request_args: {:?}", request_args);
